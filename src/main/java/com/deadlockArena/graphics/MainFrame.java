...
import com.deadlockArena.constant.SpringUtils;
	...
	public void addSelectButtons(Game game, SelectGrid selectGrid) throws Exception {
		...
		for (int i = 0; i < selectGrid.getJButtons().length; i++) {
			for (int j = 0; j < selectGrid.getJButtons() [ i ].length; j++) {
				Optional<Champion> c = jpaGetData.getChampion(JavaData.CHAMPIONS [ i ] [ j ]);
				if (c.isPresent()) {
					SelectButton selectButton = new SelectButton(game, c.get());
					selectGrid.setJButton(i, j, selectButton);
					this.panelWest_a.add(selectButton, gbc);
					gbc.gridx++;
				} else {
					throw new Exception("Champion not present");
				}
			}
			gbc.gridy++;
			gbc.gridx = 0;
